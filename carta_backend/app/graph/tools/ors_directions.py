"""OpenRouteService Directions + Overpass fuel-stop tool for the map_scraper agent."""
...
async def find_fuel_stops_on_route(
    ...
    """Find fuel / gas stations near the midpoint of a driving route.

    Uses OpenRouteService Directions API to compute the route geometry and find
    the geographic midpoint, then queries Overpass (OpenStreetMap) for
    amenity=fuel nodes within 1000 m of that midpoint. No Google Maps required.
    ...
    # ---------------------------------------------------------------------------
    # Step 1: Get route geometry from OpenRouteService
    # ---------------------------------------------------------------------------
    mid_lat = (origin_lat + dest_lat) / 2
    mid_lng = (origin_lng + dest_lng) / 2  # fallback if ORS fails
    ...
    try:
        async with httpx.AsyncClient(timeout=12.0) as client:
            resp = await client.post(
                ...
                json={
                    "coordinates": [
                        [origin_lng, origin_lat],   # ORS uses [lng, lat]
                        [dest_lng, dest_lat],
                    ],
                    "geometry": True,
        ...
        # Extract midpoint from GeoJSON geometry coordinates
        coords = (
    ...
    # ---------------------------------------------------------------------------
    # Step 2: Query Overpass for fuel nodes within 1000 m of midpoint
    # ---------------------------------------------------------------------------
    query = f"""
[out:json][timeout:15];
node["amenity"="fuel"](around:1000,{mid_lat},{mid_lng});
out body 5;
"""
    try:
